...
pthread_cond_t condition_var = PTHREAD_COND_INITIALIZER; // let threads wait until some events eccur
...
int main(int argc, char **argv){
    ...
    while (true) {
        ...
        *p_client = client_socket;
        ...
        enqueue(p_client);
        pthread_cond_signal(&condition_var); // let one of the waiting threads stops waiting
...
void* thread_function(void* arg){
    while (true) {
        int *p_client;
        pthread_mutex_lock(&mutex);
        // condition variables are designed to work with mutex
        // when the tread is suspended, it also releases the lock and allows other threads to access critical section
        // the threads only wait if the can't get work from the queue
        // this prevents threads from calling wait() every time they finish work
